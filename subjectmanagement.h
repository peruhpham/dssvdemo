...
void resetMenuSubject(int &selectedItem){
	int y = MENU_SUB_SY; // V? trí y c?a menu d?u tiên
    ...
    while (1) {
        ...
        if (kbhit()) {
            ...
            key = getch();
            // cleardevice(); // Xóa màn hình d? v? l?i menu
            if (key == 72) {// X? lý phím lên
                selectedItem = (selectedItem - 1 + 3) % 3;
            }
            else if (key == 80) {// X? lý phím xu?ng
                selectedItem = (selectedItem + 1) % 3;
            }
            // In l?i menu v?i l?a ch?n m?i du?c tô sáng
            y = MENU_SUB_SY;
			setcolor(BLACK);
            outtextxy(MENU_SUB_SX, 10 + y, ">> Danh sach mon theo ID");
            outtextxy(MENU_SUB_SX, 10 + y + MENU_ITEM_HEIGHT, ">> Danh sach mon theo Ten");
            outtextxy(MENU_SUB_SX, 10 + y + 2 * MENU_ITEM_HEIGHT, ">> Nhap mon hoc");
            // Tô sáng l?a ch?n hi?n t?i
			if(selectedItem == 0){
				setcolor(BLUE);
            	outtextxy(MENU_SUB_SX, 10 + y, ">> Danh sach mon theo ID");
			}
            ...
            setcolor(WHITE); // Reset màu v? m?c d?nh
        }
	}
	setDefault();
}
...
// Phan xu lý/////////////////////////////////////////
//----------------------------------------------------------------
// Hàm v? menu và c?p nh?t l?a ch?n
void drawMenuAndUpdateSelection(listSubject &lsb, int &selectedItem) {
    int y = MENU_SUB_SY; // V? trí y c?a menu d?u tiên
...
// 			// S?p xDanh sách t?m th?i theo tên môn h?c
// 			// sapXepChon(danhSachTamThoi, currentIndex, soSanhTheoTenMonHoc);

// 			// Danh sách môn h?c theo th? t? tên môn h?c
// 			cout << "Danh sach mon hoc theo thu tu ten:" << endl;
